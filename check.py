...
def get_data(batch_size, data_root, num_workers=4, data_augmentation=False):
    ...
    train_loader = subset_data_loader(train_data_path, 
                               train_transforms, 
                               batch_size=batch_size, 
                               shuffle=False, # shuffle=True  --- a mistake?
                               num_workers=num_workers)
...
def get_sample_prediction_errors(model, data_root, mean, std):
    batch_size = 15
    ...
    for j in range(interval):
        ...
        for i in range(batch_size):
            ...
            trans_input, target = test_dataset_trans.__getitem__(i + index)
            img, _ = test_dataset.__getitem__(i + index)
        ...
        for i, target in enumerate(targets_):
            if (test_dataset.classes[cls[i]] != test_dataset.classes[targets_[i]]):
                imgs.append(imgs_[i])
                guess.append(test_dataset.classes[cls[i]])
                truth.append(test_dataset.classes[targets_[i]])
                probs.append(prob[i])
                targets.append(targets_[i])
    ...
    for i, target in enumerate(targets):
        img = transforms.functional.to_pil_image(imgs[i])
        plt.imshow(img)
        plt.gca().set_title('P:{0}({1:.2}), T:{2}'.format(guess[i], 
                                                          probs[i], 
                                                          truth[i]))
        plt.show()
        
    #fig.savefig('sample_prediction_errors.png')
...
if __name__ == '__main__':
    ...
    for i in range(len(train_data.classes)):
        img, target = train_data[int((i * len(train_data)) / len(train_data.classes))]
